...
const Chat: React.FC = () => {
  ...
  const { conversations, fetchChats, renameChat, deleteChat } = useChats();
  ...
  // Prioriser le titre du store local pour les mises à jour en temps réel
  ...
  // Rendu unifié pour tous les cas
  return (
    <div className="chat-wrapper flex flex-col h-screen overflow-y-auto">
      {/* Fixed Header for Chat - uniquement si on a un sessionIdParam */}
      {sessionIdParam && (
        <ChatFixedHeader
      ...
      <div className="flex-wrapper flex-1 flex flex-col h-full">
        <div
          className={`flex-1 ${
            isHomePage ? "pt-20 md:pb-36 pb-28 overflow-hidden" : ""
          }`}
        >
